-- Interruptor reversível do IRPF por household: households.ir_enabled.
-- Default TRUE → comportamento atual preservado pra todo mundo. Quem setar FALSE
-- some com a superfície de IRPF no app (hub, páginas, accordions, auto-promoção,
-- notificações) sem perder NENHUM dado. Religa com ir_enabled = true.
--
-- Mantém intacto o IR de RESGATE (lib/financial/tax.ts) — é cálculo leve na hora
-- de sacar, não papelada de declaração.

alter table public.households
  add column if not exists ir_enabled boolean not null default true;

comment on column public.households.ir_enabled is
  'IRPF (declaração) ligado pro household. False esconde a superfície de IRPF e '
  'desliga a auto-promoção de deduções. Não afeta o IR de resgate.';

-- Gate na auto-promoção de deduções: só promove pra ir_deductible_payments se o
-- household tiver IRPF ligado. Resto da função idêntico à versão com guarda de
-- idempotência (20260603160000).
...
begin
  ...
  -- IRPF ligado pro household? (gate da auto-promoção de deduções)
  select ir_enabled into v_ir_enabled from public.households where id = v_rule.household_id;
  v_ir_enabled := coalesce(v_ir_enabled, true);
  ...
  while v_cursor <= v_end loop
    ...
    -- GUARDA DE IDEMPOTÊNCIA: não recria se já existe transação dessa regra+data.
    if exists (
    ...
    if v_rule.kind = 'transfer' then
      ...
      -- Auto-promove pra ir_deductible_payments (só se IRPF ligado e não exclude)
      if v_ir_enabled
         and v_rule.kind = 'expense'

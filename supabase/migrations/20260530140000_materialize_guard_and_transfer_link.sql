-- ============================================================================
-- materialize_recurrence: restaura guard do last_materialized_date + vincula
-- transferências recorrentes por transfer_pair_id
-- ============================================================================
-- Dois bugs da auditoria, ambos na função unificada 20260528160000:
--
-- (1) REGRESSÃO (recorrentes#0): a unificada voltou a avançar
--     last_materialized_date INCONDICIONALMENTE (update sem `if v_count > 0` e
--     sem checar `v_end > last_materialized_date`), revertendo o fix da
--     20260526020000. Isso causa:
--       - RETROCESSO: materializar até uma data anterior à já materializada
--         puxa last_mat pra trás → próxima materialização DUPLICA ocorrências.
--       - AVANÇO PREMATURO: regra com start_date futuro tem last_mat populado.
--     Fix: só avança quando criou algo (v_count > 0) E nunca retrocede
--     (v_end > last_materialized_date). A fatura R$0 (v_count=0) continua sendo
--     re-tentada no próximo ciclo — idempotente e barato.
--
-- (2) TRANSFER NÃO LINKAVA (schema-db#16): o UPDATE pós-create_transfer casava
--     por `description = v_rule.description`, mas create_transfer grava a
--     descrição com seta + nome da conta ("X → Conta"), então NUNCA casava — a
--     tx ficava sem recurring_rule_id/is_recurring e sem herdar
--     exclude_from_ir/is_historical_ir_only. Fix: capturar o transfer_pair_id
--     retornado por create_transfer e casar por ele (determinístico).
-- ============================================================================
...
comment on function public.materialize_recurrence is
  ...
  '(2) pagamento de fatura usa valor real via credit_card_bill_amount(); '
  '(3) propaga fonte/IRRF/INSS/exclude_from_ir; '
  '(4) auto-promove dedutíveis; '
  '(5) transfer linkada por transfer_pair_id; '
  '(6) last_materialized_date só avança quando v_count>0 e nunca retrocede.';

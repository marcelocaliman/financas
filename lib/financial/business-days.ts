...
/**
 * Fração do dia útil corrente (em SP) já transcorrida — 0 a 1.
 * Se hoje não é dia útil, retorna 0.
 */
export function todayBusinessProgress(now: Date = new Date()): number {
...
/**
 * Conta dias úteis fracionários entre `fromISO` (inclusive) e `now`
 * (exclusive), considerando feriados nacionais brasileiros.
 *
 * Retorna float:
 *  - parte inteira = dias úteis completados entre from e ontem
 *  - parte fracionária = fração do dia atual já transcorrida (se hoje for útil)
 *
 * Estratégia: itera dia a dia (até ~365 iterações típicas, ok pra checkpoint
 * tipicamente recente).
 */
export function businessDaysSinceContinuous(fromISO: string, now: Date = new Date()): number {
  ...
  if (fromDate >= todayUTC) {
    // from no futuro ou hoje: só conta a fração do dia (se hoje é útil)
    return todayBusinessProgress(now);
  }

  // Conta dias úteis completos de (fromDate, todayUTC) — exclusivo nos dois extremos
  // Ou seja: dias úteis ENTRE from+1 e ontem (inclusive).
  let count = 0;
  ...
  while (cursor < todayUTC) {
    ...
    if (wd !== 0 && wd !== 6) {
      ...
      if (!holidaysForYear(y).has(dateKey(y, m, d))) {
        count += 1;
      }
    }
    cursor.setUTCDate(cursor.getUTCDate() + 1);
  ...
  // Soma a fração do dia atual, se hoje é útil
  return count + todayBusinessProgress(now);
}

...
export default function Painel() {
  ...
  if (!view) return <div className="h-[60vh] rounded-[18px] bg-card border border-border animate-pulse" />;
  ...
  return (
    <div>
      {/* ── HERO full-page ─────────────────────────────────────────── */}
      <div className="min-h-[calc(100svh-4rem)] flex flex-col justify-center py-12">
        <Eyebrow>{name ? t("common.hello", { name }) : t("dashboard.netWorth")}</Eyebrow>
        <HeroNumber
          value={view.netWorth}
          currency={disp}
          className="block text-[clamp(56px,11vw,148px)] mt-3"
        />
        {hasTrend ? (
          <div className="mt-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mt-12 max-w-5xl">
        ...
        <button
          ...
          className="mt-12 self-start inline-flex items-center gap-1.5 text-[12px] font-medium text-faint hover:text-text transition-colors"
        >
          {t("dashboard.scroll")}
      ...
      <div className="grid grid-cols-12 gap-5 pt-2">
        <Tile className="col-span-12 lg:col-span-8 p-6 flex flex-col">
          ...
          <div className="w-full flex-1 min-h-[190px] mt-3">
        ...
        <Tile className="col-span-12 lg:col-span-4 p-6">
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-5 mt-4">
            <div className="w-[110px] h-[110px] shrink-0">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={view.expDisp} dataKey="value" nameKey="name" innerRadius={34} outerRadius={53} paddingAngle={2} stroke="none">
            ...
            <div className="flex-1 space-y-1.5 min-w-0">
              {view.expDisp.slice(0, 5).map((e, i) => (
        ...
        <Tile className="col-span-12 p-6">
...
function StatTile({ label, value, sub, positive }: { label: string; value: string; sub?: string; positive?: boolean }) {
  return (
    <Tile className="p-6">
      ...
      <div className={cn("font-display font-semibold text-[clamp(22px,2.2vw,30px)] tracking-[-0.02em] tabular mt-2", positive ? "text-pos" : "text-text")}>
        {value}
      </div>
...
function PainelEmpty() {
  ...
  return (
    <div className="min-h-[calc(100svh-4rem)] flex flex-col items-center justify-center text-center">
      <div className="w-12 h-12 rounded-2xl bg-accent-soft text-accent flex items-center justify-center mb-5">
        <Sparkles size={22} />
      </div>
      <div className="text-[clamp(24px,4vw,40px)] font-display font-semibold tracking-[-0.02em]">{t("dashboard.empty")}</div>

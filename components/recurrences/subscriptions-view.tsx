...
/**
 * View dedicada de assinaturas — KPIs focados em drainage, próximas
 * cobranças, lista ordenada por custo, e verdict 10-anos. Renderizada
 * quando o pill "Assinaturas" está ativo em /recorrentes.
 */
export function SubscriptionsView({ subs }: { subs: Subscription[] }) {
  if (subs.length === 0) {
    return <EmptyState />;
  }
        ...
        <div className="hidden lg:block overflow-x-auto px-7">
          <table className="w-full">
            ...
            <tbody>
              {subs.map((s) => {
                const sharePct =
                  monthlyTotal > 0 ? (s.monthlyInDisplay / monthlyTotal) * 100 : 0;
                return (
                  <tr
                    key={s.id}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="font-mono text-[10.5px] text-faint-foreground tracking-[0.04em] mt-0.5">
                      {freqLabel(s.frequency, s.interval_count, s.day_of_month)}
                      {s.nextChargeDate
                        ? ` · próx ${formatDateShort(s.nextChargeDate)}`
                        : ""}
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="font-mono text-[14.5px] font-medium tabular-nums">
                      <MoneyMask>{formatMoney(s.monthlyInDisplay)}</MoneyMask>
                      <span className="text-[10.5px] text-faint-foreground ml-0.5">
                        /mês
                      </span>
                    </div>
      ...
      {monthlyTotal > 0 ? (
        <Panel className="mt-6 !p-5">
          <div className="flex items-start gap-3">
            <AlertCircle
              className="w-5 h-5 text-gold-700 dark:text-gold-500 shrink-0 mt-0.5"
              strokeWidth={1.7}
            />
    ...
    <Panel className="!py-14 grid place-items-center text-center">
      <div className="max-w-[480px]">
        ...
        <h2 className="font-display text-[24px] tracking-[-0.02em] mt-2">
          Você não tem assinaturas mapeadas.
        </h2>
        <p className="text-[13.5px] text-muted-foreground mt-2.5 leading-relaxed">
          Use o botão <b>&ldquo;Nova assinatura&rdquo;</b> acima pra cadastrar a
          primeira, ou troque pra outra pill pra cadastrar uma recorrência
          normal — quem tem nome conhecido (Netflix, Spotify, Claude, gym…) é
